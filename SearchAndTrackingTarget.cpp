    ...
    if(_contours.size()>0)
    {
        ...
        for(size_t i=0; i<_contours.size(); i++)
        {
                    ...
                    rectEngine.height>=_minSizeOfTarget.height && rectEngine.width>= _minSizeOfTarget.width)
            {
                ...
                Rect rectRegion = createRegion(rectEngine, _original, 1);
                Mat region = _original(rectRegion).clone();
                rectEngine.x -= rectRegion.x;
                rectEngine.y -= rectRegion.y;

                rectEngine = createRegion(rectEngine, region, 0);
                ...
                if(_typeOfCamera ==0 && gl_debug)
                {
                    ...
                    imshow("BigRegion", _original(createRegion(rectRegion, _original, 2)));
...
void SearchAndTrackingTarget::follow1()
{
    if(_typeOfCamera ==0 && gl_debug)
    {
        imshow("cannyOut", _roi);
        qDebug()<<maxBrightness(_roi);
        waitKey();
        QThread::sleep(1);
    }

    _positionTarget = maxBrightness(_rectangleRoi);
    createRoi(_delta);
...
void SearchAndTrackingTarget::search()
{
    ...
    for(size_t i = 0; i < _contours.size(); i++)
    {
                ...
                _rectangleTarget.height>=_minSizeOfTarget.height && _rectangleTarget.width>= _minSizeOfTarget.width)
        {
            ...
            Rect rectRegion = createRegion(_rectangleTarget, _original, 1);
            Mat region = _original(rectRegion).clone();
            Rect rectEngine;
...
Rect SearchAndTrackingTarget::createRegion(Rect baseRect, Mat originalImg, float k)
{
    Rect result;
    ...
    if(result.x+result.width>originalImg.cols)
    {

        result.width-=(result.x+result.width-originalImg.cols);

    }
    if(result.y+result.height>originalImg.rows)
    {
        result.height-=(result.y+result.height-originalImg.rows);
    }
...
void SearchAndTrackingTarget::setTypeOfCamera(int cameraType)
{
    ...
    if(cameraType==0)
    {
        _maxSizeOfTarget.width=30;
        _maxSizeOfTarget.height = 30;
...
//    _rectangleRoi.x = _rectangleTarget.x - delta/2;
    ...
    _rectangleRoi.height = delta;
    _rectangleRoi.width =  delta;
    _rectangleRoi.y = _positionTarget.y - delta/2;
    _rectangleRoi.x = _positionTarget.x - delta/2;
...
{    
    if(isFounded())
    {
        ...
        _treshold1 = meanValue*0.1;
        _treshold2 = meanValue*1;
    }
    else
    {
